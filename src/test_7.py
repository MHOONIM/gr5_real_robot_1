...
class searching_test():
    ...
    def __init__(self):
        ...
        # initialise the destination flag
        self.locate = False
        self.dest_flag = 0
        self.get_target(self.dest_flag)
    ...
    # ---------- Get the next destination to explore ---------
    def get_target(self, destination):
        if destination == 0:
            ...
            self.target_x = -1.1
            self.target_y = self.tb3_odom.y - 1.2
        elif destination == 2:
            self.target_x = self.tb3_odom.x + 1.2
            self.target_y = self.tb3_odom.y
    # --------------------------------------------------------
    ...
    def main_loop(self):
        ...
        while not self.ctrl_c:
            ...
            if beta < 0:
                beta = pi + (pi + beta)
            alpha = math.atan((self.target_y - self.tb3_odom.y) / (self.target_x - self.tb3_odom.x))
            if (self.target_y - self.tb3_odom.y < 0) and (self.target_x - self.tb3_odom.x < 0):
                alpha = alpha + pi
            if (self.target_y - self.tb3_odom.y < 0) and (self.target_x - self.tb3_odom.x > 0) > 0:
                alpha = alpha + 2 * pi
            gam = beta - alpha
            ...
            if self.locate == False: 
                if gam > 0.1:
                    self.vel.linear.x = 0
                    self.vel.angular.z = gam * -1
                    if self.vel.angular.z > 1:
                ...
                if (self.tb3_odom.x > self.target_x + 0.2 or self.tb3_odom.x < self.target_x - 0.2) or (self.tb3_odom.y > self.target_y + 0.2 or self.tb3_odom.y < self.target_y - 0.2):
                    ...
                    self.vel.linear.x = 0
                    self.vel.angular.z = 0
                    self.dest_flag = self.dest_flag + 1
                    self.get_target(self.dest_flag)
                    self.locate = False
